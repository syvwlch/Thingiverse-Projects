...
{ 
	...
	polyTriangles=[
		...
		[7,6,13], [13,14,7], [8,7,14], [14,15,8], 			// bottom of thread
		//[3,4,5], [5,4,6], [5,6,7], 					// A side of thread
		//2[11,10,12], [11,12,13], [12,14,13] 				// B side of thread
			];
...
{ 
	polyPoints=[
		...
		barycenter(ATop,ABottom,AThreadPosition+AThreadRatio/2) 
			+ unitVector(ATop-ABottom)*AThreadDepth/2*tan(AThreadAngle),

		barycenter(ATop,ABottom,AThreadPosition+AThreadRatio/2) 
			- unitVector(ATop-ABottom)*AThreadDepth/2*tan(AThreadAngle) 
			+ unitVector(AThroatCenter-0.5*(ATop+ABottom))*AThreadDepth,
		...
		barycenter(BTop,BBottom,BThreadPosition+BThreadRatio/2) 
			+ unitVector(BTop-BBottom)*BThreadDepth/2*tan(BThreadAngle),

		barycenter(BTop,BBottom,BThreadPosition+BThreadRatio/2) 
			- unitVector(BTop-BBottom)*BThreadDepth/2*tan(BThreadAngle) 
			+ unitVector(BThroatCenter-0.5*(BTop+BBottom))*BThreadDepth,
		...
		)
{
	...
	slice( 
		...
		AThreadDepth= 	min((0+min(i,steps-i))/stepsPerTurn/startLength,1)*threadHeight(i), 
		AThreadRatio= 	min((1+min(i,steps-i))/stepsPerTurn/startLength,1)*profileRatio(i),
		AThreadPosition= 	threadPosition(i),
		AThreadAngle= 	threadAngle(i),
		
		BThreadDepth= 	min((0+min(i+1,steps-i-1))/stepsPerTurn/startLength,1)*threadHeight(i+1),
		BThreadRatio= 	min((1+min(i+1,steps-i-1))/stepsPerTurn/startLength,1)*profileRatio(i+1),
	...
	if (RH==true)
	for (i=[-stepsPerTurn:0])
	{
	sliceBottom( 
		AShaftBottom= 	[ShaftX(i/steps),				ShaftY(i/steps),				Z(0)						],
		AShaftTop= 	[ShaftX((i+stepsPerTurn)/steps),	ShaftY((i+stepsPerTurn)/steps),	ShaftZ((i+stepsPerTurn)/steps)	],
		BShaftBottom= 	[ShaftX((i+1)/steps),			ShaftY((i+1)/steps),			Z(0)						],
		BShaftTop= 	[ShaftX((i+1+stepsPerTurn)/steps),	ShaftY((i+1+stepsPerTurn)/steps),	ShaftZ((i+1+stepsPerTurn)/steps)	],
		ABottom= 		[X(i/steps),					Y(i/steps),					Z(0)						],
		ATop= 		[X((i+stepsPerTurn)/steps),		Y((i+stepsPerTurn)/steps),		Z((i+stepsPerTurn)/steps)		],
		BBottom= 		[X((i+1)/steps),				Y((i+1)/steps),				Z(0)						],
		BTop= 		[X((i+1+stepsPerTurn)/steps),		Y((i+1+stepsPerTurn)/steps),		Z((i+1+stepsPerTurn)/steps)		],
	...
	if (RH==true)
	for (i=[steps:steps+stepsPerTurn-1])
	{
	sliceTop( 
		AShaftBottom= 	[ShaftX(i/steps),				ShaftY(i/steps),				ShaftZ(i/steps)				],
		AShaftTop= 	[ShaftX((i+stepsPerTurn)/steps),	ShaftY((i+stepsPerTurn)/steps),	ShaftZ(1)+pitch				],
		BShaftBottom= 	[ShaftX((i+1)/steps),			ShaftY((i+1)/steps),			ShaftZ((i+1)/steps)			],
		BShaftTop= 	[ShaftX((i+1+stepsPerTurn)/steps),	ShaftY((i+1+stepsPerTurn)/steps),	ShaftZ(1)+pitch				],
		ABottom= 		[X(i/steps),					Y(i/steps),					Z(i/steps)					],
		ATop= 		[X((i+stepsPerTurn)/steps),		Y((i+stepsPerTurn)/steps),		Z(1)+pitch					],
		BBottom= 		[X((i+1)/steps),				Y((i+1)/steps),				Z((i+1)/steps)				],
		BTop= 		[X((i+1+stepsPerTurn)/steps),		Y((i+1+stepsPerTurn)/steps),		Z(1)+pitch					],
		...
		)
{
	...
	function 	ShaftX(i)= 			0*pitchRadius*cos(i*360*numberTurns); 
	function 	ShaftY(i)= 			0*pitchRadius*sin(i*360*numberTurns);
	function 	ShaftZ(i)= 			pitch*numberTurns*i;
	...
	function 	ThroatX(i)=			throatDistance*cos(i*360*numberTurns);
	function 	ThroatY(i)=			throatDistance*sin(i*360*numberTurns);
	function 	ThroatZ(i)=			throatHeight;
	...
	sliceThroat( 
		...
		AThroatCenter=	[ThroatX(i/steps),				ThroatY(i/steps),				ThroatZ(i/steps)				],
		BThroatCenter=	[ThroatX((i+1)/steps),			ThroatY((i+1)/steps),			ThroatZ((i+1)/steps)			],
